...
supabase/migrations/008_security_compose_and_android_options.sql
supabase/migrations/009_admin_system_settings.sql
supabase/migrations/010_backend_bridge.sql
```
...
## 4.8 Migration 008 — Security, Compose, dan Android options

Menambahkan trusted Compose access, mode build Compose, min/target SDK, architecture, data folder, audit logs, payment events, dan security foundation.

## 4.9 Migration 009 — Admin system settings

Menambahkan maintenance mode, kontrol registrasi, kontrol build, announcement, dan global settings yang hanya dapat diubah melalui admin API.

## 4.10 Verifikasi table
...
## 4.11 Jangan menjalankan migration acak
...
- [ ] Migration 008 berhasil
- [ ] Migration 009 berhasil
- [ ] Table `builds` lengkap
- [ ] Table `audit_logs` tersedia
- [ ] Table `system_settings` tersedia
...
# 26. Cloudflare Turnstile dan Upstash Rate Limit

Buat widget Turnstile di Cloudflare Dashboard dan tambahkan domain production. Isi Vercel:
...
Turnstile bersifat optional-by-env. Jika kedua key belum diisi, website tetap berjalan. Setelah secret aktif, register, login, dan build memerlukan token valid.

Buat Redis database di Upstash dan isi:
...
Limiter melindungi register, login, build, dan download. Jangan memasang token Upstash pada `NEXT_PUBLIC_*`.

Untuk alert volume abnormal, buat Discord-compatible webhook:

```env
SECURITY_ALERT_WEBHOOK=https://...
```

# 27. Trusted Jetpack Compose Builder

Compose Builder menerima project Android Studio/Gradle ZIP hanya untuk admin atau profile dengan `compose_access=true`. Admin dapat mengaktifkannya dari Admin Control Center.

Keamanan pipeline:

1. Source diunduh memakai signed URL.
2. ZIP diperiksa dari path traversal dan symbolic link.
3. ClamAV memindai archive dan hasil ekstraksi.
4. Gradle project berjalan pada job `build-untrusted` tanpa Supabase service-role atau signing key.
5. APK artifact dipindahkan ke job `publish` terpisah.
6. Signing dan upload dilakukan setelah untrusted Gradle process selesai.
...
gradlew
gradle/wrapper/
app atau module Android yang menghasilkan release APK
```

Full Gradle project tetap berisiko dan hanya boleh diberikan kepada akun tepercaya. Jangan mengaktifkan Compose access massal.

# 28. API Domain dan Signed APK Download
...
```text
GET https://api.xystudio.my.id/
GET https://api.xystudio.my.id/download/{buildId}?e=...&s=...
POST https://api.xystudio.my.id/webhooks/midtrans
```

Download link memakai HMAC dan expiry. URL token tetap sulit ditebak, tetapi nama file mengikuti nama dan versi aplikasi, misalnya `TokoKita-Pro-v1.2.0.apk`.

# 29. Midtrans Webhook Verification
...
Plan hanya diaktifkan pada status `settlement` atau `capture`, dan event disimpan pada `payment_events`. Saat membuat transaksi, masukkan user UUID pada `custom_field1` dan plan `pro`/`business` pada `custom_field2`. Gunakan HTTPS dan jangan pernah menerima perubahan plan langsung dari browser.

# 30. Help Center, Legal, dan Cookie Consent

Help Center mencakup getting started, guides, APK, HTML format, permission, push, storage, signing, update, troubleshooting, changelog, status, contact, API, Acceptable Use, Refund, Copyright/DMCA, dan Service Level.

Cookie banner memiliki:

- Terima semua
- Tolak opsional
- Pengaturan kategori
- Cookie esensial selalu aktif
- Analytics baru dimuat setelah consent
- Tombol pengaturan ulang di kiri bawah

Tambahkan `help.xystudio.my.id` ke Vercel dan isi `NEXT_PUBLIC_HELP_URL` agar seluruh link legal menggunakan subdomain tersebut.

# 31. Email Perusahaan Gratis

Untuk menerima email seperti:
...
Gunakan Cloudflare Email Routing pada paket gratis. Domain DNS harus dikelola Cloudflare. Buat destination ke Gmail/Outlook pribadi dan aktifkan route per alamat. Email Routing adalah forwarding, bukan mailbox penuh.

Untuk email keluar/SMTP dan Supabase Auth, gunakan Resend/Brevo free tier. Tambahkan SPF, DKIM, dan DMARC. Jangan membuat MX record ganda yang bertentangan.
